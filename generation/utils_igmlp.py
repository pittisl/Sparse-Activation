    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        for name, module in model.named_modules():
            if name == "model.layers."+str(mask_layer_list[0])+".mlp.mlp_layer":
        ...
        while True:
            ...
            if number_forward >= 0:
                 ...
                 #print('fffffffffffffffffffffffffffffffffffffff', features_out_hook[0][0,0,0,0],features_out_hook[1][0,0,0,0])
                 ...
                 for layer_number in mask_layer_list:
                     for n in range(n_s):
                         #print((torch.full_like(target_tensor, fill_value=(2-n)/2)+n*para/2).shape)
                     
                         features_out_hook = []

                         mlp_m=torch.full_like(target_tensor, fill_value=(n+1)/n_s)
                         #print(head_m)
                         ...
                         **model_inputs,
                         ...
                         output_attentions=output_attentions,
                         output_hidden_states=output_hidden_states,
                         mlp_mask_tesnor = mlp_m,               #############torch.mul(heads_mask, (2-n)/2+n/2)
                         ...
                         #print(n, number_forward)
                         #print(pre.shape, solution.shape)
                         #print(pre, solution)
                         ...
                         #else:
                             #print('repreprepreprepreprepreprepreprepreprep', len(features_out_hook), features_out_hook[0][0,0,0,0])
                         #print('loss', loss)
                         #print('feature', features_out_hook[0][0,0,0,0])
                         #print('len', len(features_out_hook))



                     #print('dddddddddddddddddddddddddddddddddddddddddddd', number_forward)
                     #grad=grad/n_s
                     grad=torch.mul(grad, feat)
                     #print(feat[0,0,0,0])
                     #print('shae', grad.shape)
                     gradient_layer_list.append(grad/n_s)      ###############(torch.mean(torch.mul(grad/n_s, feat), dim=3))

                 if len(gradient_layer_list)>0:
                     gradient_list.append(torch.stack(gradient_layer_list,dim=1))
                    #print('dffsdfsdfsdfsd', torch.stack(gradient_layer_list,dim=1).shape)
                    #for thre in range(24):

                    #print(len(gradient_layer_list), gradient_list[0].shape)
        ...
        ig_tensor=torch.stack(gradient_list,dim=0)            ############################################################################################################# modification
        hook_handle.remove()
