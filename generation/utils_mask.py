...
class GenerationMixin:
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        #generate_length=sol.shape[1]
        
        while True:
            #if number_forward>generate_length:   ############################################################################################################# modification
                #break                            ############################################################################################################# modification

            number_forward=number_forward+1         ############################################################################################################# modification
            #print(number_forward, generate_length)
            ...
            #print(number_forward)
            #print(len(heads_mask))
            #print(len(mlp_mask))
            #print(sol.shape)
            ...
            if number_forward<1:
               hm_tensor=None
            elif number_forward >= heads_mask.shape[0]:
               hm_tensor=None 
            elif heads_mask is not None:
               hm_tensor=heads_mask[number_forward,:,:]  #heads_mask[number_forward,:,:]
            else:
               hm_tensor=None 
            ...
            if number_forward<1:
               ...
               mlp_tensor=None
            elif mlp_mask is not None:
               mlp_tensor=mlp_mask[number_forward,:,:]  #heads_mask[number_forward,:,:]  
            else:
               mlp_tensor=None 
            ...
            outputs = model(
                **model_inputs,
                ...
                heads_mask_tesnor = hm_tensor,  #heads_mask[number_forward,:,:],
                mlp_mask_tesnor = mlp_tensor,  #heads_mask[number_forward,:,:],
                layer_ind_list = mask_layer_list,
            )
